// stego.js - General File-to-File Steganography (EOF Injection)
...
function updateEmbedUI() {
    ...
    const hasPayload = isTextMode ? inputStegoMessage.value.trim().length > 0 : !!secretFile;
...
btnEmbed.addEventListener('click', async () => {
    try {
        ...
        // Let the UI paint the "Encrypting..." state
        ...
        const finalFileBlob = new Blob([embedFile, finalPayload], { type: embedFile.type });
        ...
        if (nameParts.length > 1) {
            ...
            outName = nameParts.join('.') + '_secure.' + ext;
        }
        ...
        btnEmbed.appendChild(icon);
        btnEmbed.appendChild(document.createTextNode(' Encrypt & Embed'));
        updateEmbedUI();
    }
});
...
btnExtract.addEventListener('click', async () => {
    try {
        ...
        if (!extractedPayload) {
            throw new Error("No hidden data found in this file (Marker missing).");
        }
